...
const formatStaff = (profile, user) => {
  ...
  return {
    ...
    // What the Permissions tab should draw as ticked — resolved, not stored.
    // Covers the portals a role opens and everything full administrator access
    // carries, neither of which appears in the stored lists. See
    // displayedPermissions() for why this is separate from the gate's answer.
    effectivePermissions: displayedPermissions(user),
